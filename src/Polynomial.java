...
public class Polynomial extends ArrayList<Integer> {
		...
		for (int i = 0; i < this.size() - 1; i++)  {
			...
			if (coeff != 0) {
				...
				if (this.size()-1-i == 1) {
					...
					if (coeff == 1) {
						result = result + "x^" + (this.size()-i);
					} else {
						result = result + coeff + "x^" + (this.size()-i);
					}
				}
				if (this.get(i + 1) < 0) {
					result += " - ";
				} else if (this.get(i + 1) > 0) {
					result += " + ";
				}
			}
		}	
		coeff = Math.abs(this.get(this.size()-1));
		if (coeff != 0) {
			result += coeff;
		}
		return result;
    ...
    */
	...
	// If they don't match, return falsie
	if (this.size() == p.size()) {
    ...
    */
    ...
    public static int degreeCoeffsLowToHigh(int [] coeffsLowToHigh) {
		int degree = 0;
        int i = coeffsLowToHigh.length - 1;
        while (coeffsLowToHigh[i] == 0 && i > 0) {
            i--;   
        }
        degree += i;
        return degree;
    ...
    */
    ...
    public static int degreeCoeffsHighToLow(int [] coeffsHighToLow) {
		int degree = 0;
		int i = 0;
		while (coeffsHighToLow[i] == 0 && i < coeffsHighToLow.length - 1) {
			i++;
		}
		degree = coeffsHighToLow.length - 1 - i;
		return degree;
    }
    ...
    */
    ...
    public static int [] lowToHigh(int [] coeffsHighToLow) {
    	...
    	if (degreeCoeffsHighToLow(coeffsHighToLow) == 0) {
    		int [] result = new int [1];
    		result[0] = coeffsHighToLow[length-1];
    		return result;
    	}
    	int [] result = new int [degreeCoeffsHighToLow(coeffsHighToLow) + 1];
    	for (int i = 0; i < result.length; i++) {
    		result[i] = coeffsHighToLow[length-1-i];
    	}
    	return result;
    }
    ...
    */
    ...
    public static int [] highToLow(int [] coeffsLowToHigh) {
    	...
    	if (degreeCoeffsLowToHigh(coeffsLowToHigh) == 0) {
    		int [] result = new int [1];
    		result[0] = coeffsLowToHigh[0];
    		return result;
    	}
    	int [] result = new int [degreeCoeffsLowToHigh(coeffsLowToHigh) + 1];
    	for (int i = 0; i < result.length; i++) {
    		result[degreeCoeffsLowToHigh(coeffsLowToHigh)-i] = coeffsLowToHigh[i];
    	}
    	return result;
    }
    ...
    */
    ...
    public Polynomial plus (Polynomial p) {	
		...
		for(int i = p.size(); i>0; i++)	{
			f1.add(p.size()-i, p.get(i));
		}
		for(int i = this.size(); i>0; i++) {
			f2.add(this.size()-i, this.get(i));
		}
		...
		for(int i=0; i<p.size(); i++){
			if( this.size() < i) { 
				...
				f3.add(i, f2.get(i)+f1.get(i));
			}
		}
		return f3; */
		int sum;
		Polynomial result = new Polynomial();
		int diff = p.getDegree() - this.getDegree();
		for (int i = 0; i < diff; i++) {
			result.add(i, p.get(i));
		}
		for (int j = 0; j < this.size(); j++) {
			sum = this.get(j) + p.get(j+diff);
			result.add(j+diff, sum);
		}
		return result;
    }
